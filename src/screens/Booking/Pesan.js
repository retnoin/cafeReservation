...
class Pesan extends Component {
	...
	uploadImagePayment(param, category = ''){
		let data = '';
		if(category == ''){
			data = {
				statusOrder: 'prosess',
			...
			data = {
				...
				note: this.state.dataOrder.note ? this.state.dataOrder.note:'-',
				totalPrices: this.state.dataOrder.totalPrices,
				imageUrl: ""
			}
		}
		console.log(data);
		...
		.then((res) => {
			...
			// this.props.navigation.push('payment', {orderId: this.state.dataOrder.orderId});
		})
		.catch(err => {
			console.log('Error');
		});
	...
	render() { 
		...
		let expiredDate = moment(dataOrder.expired).format("hh:mm:ss");
		let dateNow = moment(new Date()).format("hh:mm:ss");
		let orderDate = moment(dataOrder.orderDate).format("YYYY-MM-DD");
		let date = moment(new Date()).format("YYYY-MM-DD");
		if (dataOrder.imageUrl == '') {
			if (dateNow > expiredDate) {
				...
				this.uploadImagePayment(param, 'expired');
			}
		}
		return (
		<ScrollView>
				...
				(dataOrder.imageUrl != '') ? 
					...
					(dateNow > expiredDate) ? 
						<View style={{
							...
							<Text style={{ color: COLOR.white, fontSize: 18, fontWeight: "600" }}>Expired</Text>
						</View>
					: <TouchableOpacity
